...
@section('content')
    <div class="container mx-auto w-full h-full flex justify-center items-center">
        <form method="POST" action="{{ route('password.update') }}" class="md:w-1/2 xl:w-2/6 h-auto bg-white p-8 rounded shadow">
            ...
            <label for="password" class="text-sm text-gray-400 my-4 block">
                ...
                <input class="font-light hover:border-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 placeholder-gray-300 w-full p-3 rounded border border-gray-200"type="password" name="password" id="password" placeholder="Hasło jest ukryte" required autocomplete="new-password">
            </label>
            <label for="password-confirm" class="text-sm text-gray-400 my-4 block">
                <p class="mb-2 text-gray-500">Potwierdź hasło:</p>
                <input class="font-light hover:border-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 placeholder-gray-300 w-full p-3 rounded border border-gray-200"type="password" id="password-confirm" placeholder="Hasło jest ukryte" name="password_confirmation" required autocomplete="new-password">
            </label>
            <div class="mt-5 flex items-center justify-between">
                <button class="bg-indigo-500 py-2 px-3 rounded text-white text-sm hover:bg-indigo-700" type="submit">Zmień hasło</button>
            </div>
        </form>
    </div>
@endsection
